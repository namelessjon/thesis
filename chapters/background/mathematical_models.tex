...
intra- and extracellular ion concentrations, reservoirs and other
details.
...
\subsection{A Brief History of Cardiac Myocyte Modelling}
...
Another important concept for the finite difference approximation, and
particularly for the forward Euler approximation, is that of
stability~\cite{Langtangen2003}.
Stability is closely related to the accuracy of the solution.
It arises from the finite discretisation of the problem.
The choice of timestep, $\Delta t$, and space step, $\Delta x$, limits how fast
information (and thus excitation) can be transmitted in the discretised system.
The diffusion parameter, $D$, determines how fast information should be
transmitted in the system.
If information is required to be transmitted faster than the system can transmit
such information, the solution is said to be unstable.
The stability critereon for (\ref{eqn:intro:fd:partial2}) is
\begin{equation}
\label{eqn:intro:fd:stability}
\Delta t \leq \frac{{\Delta x}^{2}}{2D}
\end{equation}
Where all symbols are defined previously.
If the parameter choices do not satisfy this relationship, inaccuracies may
arise.
