...
arrhythmia via re-entrant activity~\cite{Quan1990,Zhang2003,Gonzales2003}.
...
The CV is the rate of propagation of the excitation wavei.
...
ion chanel, \ii{ANION}, could be modelled by
\begin{equation}
...
Courtemanche~et~al.~\ref{CRN98}\ atrial myocyte model and `anion' to denote the
...
figure~\ref{fig:toolkit:anion:ap}.
...
\end{center}
\caption[Anion Current AP Profiles]{
\label{fig:tookit:anion:ap}
AP profile for the CRN model in control (black) and anion (red) cases.
...
repolarisation, \apd[50]\ and \apd\ respectively.
...
in figure~\ref{fig:tookit:anion:ap}(a) and \ref{fig:tookit:anion:apdr}(b), for the
...
(b)
APDr curves for the CRN model in control and anion cases at 90\% repolarisation.
...
\ms{00}\ but the anion case starts this descent first.
...
However, despite the current's small influence on the action potential duration,
...
current leads to a higher conduction velocity.
...
excitation wave faster until, the stimulus interval reaches a critical value of
...
Both the increased vulnerability window and the reduced conduction velocity at
short pacing intervals suggests that the addition of \ii{ANION} may be
pro-arrhythmogenic.
...
used to investigate the dynamic behaviours of induced spiral wave re-entry, we can see our
hypothesis from the 1D strand results is born out.
A revolving spiral wave
...
sustained for longer due to it's shorter wavelength.
However, the reduction in wavelength is not severe enough to create a stable
mother rotor, as been observed in studies of pathological conditions.
...
the two extremes.
In the control CT case, the curves include a prominant notch at a DI of
...
the control cells, as shown in figure~\label{fig:toolkit:afer:apdr}(b).
...
\caption[AFER ERPr curves]{ERPr curves for control CT (A), control AM/PM (B), Bosch CT (C),
Bosch AM/PM (D), Workman CT (E), Workman AM/PM (F).  Both Bosch and Workman have a
significantly reduced ERPr over the whole range considered, with Bosch
having a lower ERP then Workman.  In addition, Bosch and Workman cells are still
...
Workman.  Maximal pacing rate increased from the control value of \unit{198}{bpm} to
\unit{421}{bpm} in Bosch strands and \unit{315}{bpm} in Workman strands.

The VW was reduced by AF, but in all cell types the reduction was small.
The control value of \ms{16.6} was reduced to \ms{14.2} in Bosch and \ms{14.7}
in Workman for AM and PM cell types.  The reduction in VW for CT cells was
even smaller, from \ms{15.1} in control to \ms{14.5} and \ms{14.4} in Bosch and
Workman, respectively.

\begin{figure}
\centering
%\includegraphics{figures/toolkit/afer/4_cvr}
\caption[AFER CVr curves]{CVr curves for control CT (A), control AM/PM (B), Bosch CT (C),
Bosch AM/PM (D), Workman CT (E), Workman AM/PM (F).  In each of the conditions,
...
show data under control conditions, panels ii under Bosch conditions and
panels iii under Workman conditions.  Re-entry self-terminated under control
...
properties.  AFER abbreviated \apd\ in AM cells by 66~\% in Bosch and
53~\% in Workman.
...
increased refractory period of the CT cells~\cite{Clayton2005}.  Self-termination is still
...
of the COR.
COR has no inbuilt experimental protocols however and can only be used for
single cell simulations, not tissue modelling.
