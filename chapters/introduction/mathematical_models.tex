...
specific and thus the Nernst potential applies across them.
...
$M_{i}$ and the membrane capacitance $C_m$ to give the diffusion tensor $D$.
...
The Rush--Larsen technique~\cite{Rush1978}\ is a specialised way of solving
equations such as (\ref{eqn:intro:math:dmdt}) for the gating parameters.
Since $\alpha_m$ and $\beta_m$ are dependent only the voltage, not on time, a
solution to (\ref{eqn:intro:math:dmdt})~\cite{Hodkgin1952} is:
\begin{equation}
...
\end{equation}
Using (\ref{eqn:intro:math:dmdtsoln}) is more accurate than the forward Euler
