...
using the Blender~\ref{Blender}\ 3D modelling package.
...
atrial model.
To reduce the computational time and memory requirements the atrial geometry was
divided into 10x10x10 node blocks, around 4000 of which actually had active
nodes within.
The dipole values of each node were aggregated within these blocks to form one
`large' dipole which was assumed to be positioned at the centre of mass of the
block.
Decreasing the block size to 5x5x5 nodes had a negligible effect on the final
ECG.
...
matrix.
Generating the body surface potential took one core of horace approximately 100
